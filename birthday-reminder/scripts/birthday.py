...
def add_birthday(name, date_str, year_born=None):
    ...
    if not date:
        print(f"❌ Invalid date: {date_str}. Use DD.MM. or DD.MM.YYYY")
        return False
    ...
    print(f"✅ Birthday saved: {name} - {date.day:02d}.{date.month:02d}." + (f"{year_born}" if year_born else ""))
    return True
...
def list_birthdays(upcoming_only=False, days=30):
    ...
    print("🎂 **Birthdays:**\n")
    for item in items:
        age_str = f" (turning {item['turning']})" if item["turning"] else ""
        if item["days_left"] == 0:
            print(f"🎉 **{item['name']}** - {item['date']}{age_str} - **TODAY!**")
        elif item["days_left"] == 1:
            print(f"🎈 {item['name']} - {item['date']}{age_str} - tomorrow")
        else:
            print(f"📅 {item['name']} - {item['date']}{age_str} - in {item['days_left']} days")
...
def next_birthday():
    ...
    if closest:
        ...
        if year_born:
            ...
            turning = None
        age_str = f" (turning {turning})" if turning else ""
        
        if days_left == 0:
            print(f"🎉 **{name}** has a birthday TODAY{age_str}! 🎂")
        elif days_left == 1:
            print(f"🎈 **{name}** has a birthday TOMORROW{age_str}!")
        else:
            print(f"📅 **{name}** has a birthday in {days_left} days{age_str} ({data['day']:02d}.{data['month']:02d}.)!")
...
def remove_birthday(name):
    ...
    if name in birthdays:
        del birthdays[name]
        save_birthdays(birthdays)
        print(f"✅ Birthday for {name} removed.")
        return True
    else:
...
def main():
    ...
    if args.command == "add":
        ...
        for r in reminders:
            age_str = f" (turning {r['turning']})" if r['turning'] else ""
            if r["days_left"] == 0:
                print(f"🎉 {r['name']} has a birthday TODAY{age_str}!")
            elif r["days_left"] == 1:
                print(f"🎈 {r['name']} has a birthday TOMORROW{age_str}!")
            else:
                print(f"📅 {r['name']} has a birthday in {r['days_left']} days{age_str}!")
    else:
        parser.print_help()
