...
- **NEVER** remove, overwrite, or replace existing entries. Memory files are append-only.
- If information has changed (e.g., a new API key replaces an old one), append the updated entry with today's date — do NOT delete the old one.
- If a duplicate exists, skip adding it. But never delete the original.
